...
import com.mec.util.TempViewTool;
import com.mec.util.ViewTool;
import com.mec.video.action.ICourseAction;
...
public class VideoSelectorView implements IMecView {
	...
	public void init() {
		...
		jtatCourse = new JTextArea();
		jtatCourse.setFont(normalFont);
		jpnlStudentInfo.add(jtatCourse);
	...
	public void showView() {
		...
		ICourseAction courseAction = client.getProxy(ICourseAction.class, jfrmVideoSelector);
		Course course = courseAction.getCourseByStudentId(student.getId());
		String courseName = course.getName();
		if (course.getId() != null) {
			courseName = courseName.replace("@", "\n");
		}
		jtatCourse.setText(courseName);
		
		IStudentAction studentAction = client.getProxy(IStudentAction.class);
		String photo = studentAction.getStudentPhoto(student.getPeopleId());
		byte[] photoBytes = ByteString.stringToBytes(photo);
		TempViewTool.setLabelImage(jlblStudentPhoto, photoBytes);
	}
