(function () {
	$$.htmlModel = {
		...
		'<div class="modal fade" id="{modalId=}" tabindex="-1" role="dialog" aria-labelledby="modalTopic" aria-hidden="true">'
			+ '<div class="modal-dialog {modal_style=}">'
				+ '<div class="modal-content">'
					...
					+ '<button id="{btnCloseId=}" type="button" class="btn btn-default" data-dismiss="modal">关闭</button>'
		...
		'for(normalInputGroup){<div class="input-group">'
			...
			+ '<input id="{inputId=}" type="{inputType=text}" class="form-control" placeholder="{inputPlaceholder=}">'
		+ '</div>}normalInputGroup',
		normalCarousel :
		'<div id="{carouselId=}" data-ride="carousel" data-interval="{intervalTime=5000}" class="carousel slide {carouselStyle=}">'
			...
			+ '<div class="carousel-inner">'
				+ 'for(carouselItem){<div class="item {itemActive=active}">'
					+ '<img class="{itemStyle=}" src="{imgPath=}" alt="{imgAlt=}">'
